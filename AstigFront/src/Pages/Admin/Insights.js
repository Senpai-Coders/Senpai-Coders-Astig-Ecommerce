    ...
    const loadInsightsData = async () => {
        if (adminData) {
            try {
                ...
                const earningsTod = await API.post("/admin/totalEarningsToday", { auth : getAuth()})
                    ...
                    <section className=" body-font">
                        <div className="my-9  mx-9 ">
                            <h1 className="text-teal-600 mt-4 font-lato font-thin text-3xl">
                                <span className="font-lato font-medium text-teal-400">{" ₱ "}{numberWithCommas(earning)}</span> total generated income
                            </h1>
                            <h1 className="text-teal-600 mt-1 font-lato font-thin text-2xl">
                            today's income <span className="font-lato font-medium text-teal-400">{" ₱ "}{numberWithCommas(earningToday)}</span>
                            </h1>
                            <HelperLabel
                            bg={"bg-gray-100"}
                            txtbg={"text-teal-700"}
                            isError={false}
                            msg={
                                `generated income comes from the computation of successfully delivered orders`
                            }
                        />
                        </div>
